...
function WebMByteStream(options) {
  ...
  var clearTimecodes = (typeof options.clearTimecodes === 'boolean')
    ? options.clearTimecodes : false;

  debug('options: bufferSize=' + bufferSize + ', durations='
    + durations + ', clearTimecodes=' + clearTimecodes);
      ...
      case 'Timecode':
        ...
        if (durations && null !== cluster) {
          self.emit('Media Segment', {
            cluster: cluster,
            timecode: timecode[0].value,
            duration: timecode[1].value - timecode[0].value
          });
        }
        break;
      default:
